...
def create_purchase_order():
    ...
    if len(description) > 40:
        return jsonify({'success': False, 'message': 'description cannot exceed 40 characters'}), 400
    ...
    invalid_line = next((line for line in lines if not line.get('item_id') or not line.get('unit_id') or _to_float(line.get('qty')) <= 0 or _to_float(line.get('unit_price')) <= 0), None)
    if invalid_line is not None:
        return jsonify({'success': False, 'message': 'Each line must include an item, unit, quantity > 0, and price > 0'}), 400
