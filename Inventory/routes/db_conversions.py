...
def warehouse_link_to_code(whse_link, cursor):
    ...
    cursor.execute("SELECT WhseCode FROM [_uvWarehouses] WHERE WhseLink = ?", (whse_link,))
    result = cursor.fetchone()
    return result[0] if result else None
...
def supplier_link_to_code(supplier_link, cursor):
    ...
    cursor.execute("SELECT Account FROM [_uvSuppliers] WHERE DCLink = ?", (supplier_link,))
    result = cursor.fetchone()
    return result[0] if result else None

def unit_link_to_code(unit_link, cursor):
    """Convert a unit link to its corresponding unit code."""
    cursor.execute("SELECT cUnitCode FROM [_uvUOM] WHERE idUnits = ?", (unit_link,))
    result = cursor.fetchone()
    return result[0] if result else None

def category_link_to_name(category_link, cursor):
    """Convert a category link to its corresponding category name."""
    cursor.execute("SELECT cCategoryName FROM [_uvCategories] WHERE idStockCategories = ?", (category_link,))
    result = cursor.fetchone()
    return result[0] if result else None