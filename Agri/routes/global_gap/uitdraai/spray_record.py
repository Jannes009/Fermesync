...
def fetch_spray_record_data(instruction_id):
    ...
    cur.execute("""
        SELECT
            ...
            sp.SprayPPlantDate,
            sp.SprayPHarvestDate,
            sp.SprayH80PercPetalFallDate,
    ...
    projects = [
        {
            ...
            "harvest_date": row.SprayPHarvestDate.date().isoformat() if isinstance(row.SprayPHarvestDate, datetime) else str(row.SprayPHarvestDate) if row.SprayPHarvestDate else None,
            "petal_fall_date": row.SprayH80PercPetalFallDate.date().isoformat() if isinstance(row.SprayH80PercPetalFallDate, datetime) else str(row.SprayH80PercPetalFallDate) if row.SprayH80PercPetalFallDate else None,
