...
@market_bp.route('/maintanance/create-product', methods=['POST'])
def create_product():
    # Connect to SQL Server
    conn = create_db_connection()
    cursor = conn.cursor()

    # Collect data from [mkt].the form
...
@market_bp.route('/maintanance/add-item', methods=['POST'])
def add_item():
    try:
...
@market_bp.route('/maintanance/update_tax_rate', methods=['POST'])
@login_required
def update_tax_rate():
    data = request.get_json()
...
@market_bp.route('/maintanance/', methods=['GET'])
@login_required
def maintanance_page():
    return render_template('Maintanance/maintanance.html')

@market_bp.route('/maintanance/get_agents_full', methods=['GET'])
@login_required
def get_agents_full():
    conn = create_db_connection()
    cursor = conn.cursor()
...
@market_bp.route('/maintanance/get_production_units', methods=['GET'])
@login_required
...
@market_bp.route('/maintanance/get_packhouses', methods=['GET'])
@login_required
def get_packhouses():
    conn = create_db_connection()
    cursor = conn.cursor()
...
@market_bp.route('/maintanance/get_transporters', methods=['GET'])
@login_required
...
@market_bp.route('/maintanance/update_agent', methods=['POST'])
@login_required
def update_agent_maintenance():
    data = request.get_json()
