...
@market_bp.route('/account/add_service', methods=['POST'])
@login_required
def add_service():
    ...
    try:
        ...
        if not user.check_password(account_password):
            ...
            return redirect(url_for('market.view_account'))
...
@market_bp.route('/account/remove_service/<int:service_id>', methods=['POST'])
@login_required
...
@market_bp.route('/account/update_agent', methods=['POST'])
@login_required
def update_agent():
