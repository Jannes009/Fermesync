...
window.showLinkedModal = function(delnoteNo) {
  fetch(`/market/api/linked_lines?delnote_no=${encodeURIComponent(delnoteNo)}`)
    .then(res => res.json())
    .then(lines => {
      if (!lines.length) {
...
window.toggleExpandedData = function(delLineIndex, trnConsignmentId, btn) {
  ...
  if (row.style.display === 'none') {
    ...
    if (!expandedDataContainer.innerHTML) {
      Promise.all([
        fetch(`/market/api/delivery_note_lines?del_line_index=${encodeURIComponent(delLineIndex)}`).then(res => res.json()),
        fetch(`/market/api/dockets?consignment_id=${encodeURIComponent(trnConsignmentId)}`).then(res => res.json())
...
window.unlinkConsignment = function(consignmentId, delnoteNo) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      fetch(`/market/api/unlink-consignment/${consignmentId}`, {
        method: 'POST'
      })
...
window.deleteRow = function(delnoteNo, idx, btn) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      fetch(`/market/delete_sales_entry/${salesId}`, {
        method: 'DELETE'
      })
      .then(response => response.json())
      .then(data => {
