...
async function fetchSalesOrderHeader() {
    ...
    const res = await fetch(`/market/api/sales-order/${id}`);
    if (!res.ok) throw new Error('Failed to load sales order');
    return res.json();
}

async function loadAgents() {
    const res = await fetch('/market/api/correct-invoice/agents');
...
async function loadOldProdUnitsAndInvoices() {
    if (!CM.del) return;
    const res = await fetch(`/market/api/correct-invoice/old-prod-units-and-invoices/${encodeURIComponent(CM.del)}`);
...
async function loadAllProdUnits() {
    const res = await fetch('/market/api/correct-invoice/all-prod-units');
...
async function loadAllDeliveryLines() {
    if (!CM.del) return;
    const res = await fetch(`/market/api/fetch_delivery_note_lines/${encodeURIComponent(CM.del)}`);
    const data = await res.json();
...
async function updateDeliveryLinesDisplay() {
    ...
    // Get lines to edit
    const res = await fetch(`/market/api/correct-invoice/lines-and-invoices?del_note_no=${encodeURIComponent(CM.del)}&old_prod_unit_id=${encodeURIComponent(oldId)}`);
    const data = await res.json();
    const editLines = data.lines || [];
    const invoicesWithLines = data.invoices || [];

    // Get all lines
    const allRes = await fetch(`/market/api/fetch_delivery_note_lines/${encodeURIComponent(CM.del)}`);
    ...
    // Get all invoices
    const allInvoicesRes = await fetch(`/market/api/correct-invoice/old-prod-units-and-invoices/${encodeURIComponent(CM.del)}`);
...
async function submitAgent() {
    ...
    const res = await fetch('/market/api/correct-invoice/submit-agent-change', {
        method: 'POST',
...
async function submitProd() {
    ...
    const res = await fetch('/market/api/correct-invoice/submit-produnit-change', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            del_note_no: CM.del,
...
window.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if (CM.del) {
            try {
                const res = await fetch(`/market/api/invoices-for-delivery-note/${encodeURIComponent(CM.del)}`);
