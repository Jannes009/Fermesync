...
function addSale(delNoteNo) {
    ...
    fetch(`/market/api/available-lines/${encodeURIComponent(delNoteNo)}`)
...
function showSaleDetailsModal(selectedLine, delNoteNo) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            // Submit the sales
            fetch('/market/submit_sales_entries', {
                method: 'POST',
                headers: {
...
function updateCountsDisplay(delnoteNo) {
    fetch(`/market/api/update-counts/${encodeURIComponent(delnoteNo)}`)
        .then(response => response.json())
        .then(data => {
            // Update the badges
...
window.editDeliveryHeader = function(delnoteNo) {
    ...
    fetch(`/market/api/order-status/${encodeURIComponent(delnoteNo)}`)
        .then(response => response.json())
        .then(orderStatus => {
            // Fetch current header data
            fetch(`/market/api/delivery-header/${encodeURIComponent(delnoteNo)}`)
                .then(response => response.json())
                .then(header => {
                    // Fetch dropdown options
                    Promise.all([
                        fetch('/market/api/agents').then(r => r.json()),
                        fetch('/market/api/packhouses').then(r => r.json()),
                        fetch('/market/api/transporters').then(r => r.json()),
                        fetch('/market/api/destinations').then(r => r.json())
                        ...
                        function saveHeader(dn, values) {
                            fetch(`/market/api/save-delivery-header/${encodeURIComponent(dn)}`, {
                                method: 'POST',
...
window.saveQuantityChanges = function() {
    ...
    // Save the changes
    fetch('/market/api/update-line-quantities', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        ...
        if (!row.querySelector('.delete-line-btn')) {
            ...
            btn.onclick = function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        fetch(`/market/api/delete-delivery-line/${lineId}`, {
                            method: 'DELETE'
                        })
                        .then(response => response.json())
                        .then(data => {
